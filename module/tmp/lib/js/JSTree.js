...
class JSTree {
  ...
  update() {
    // 绑定事件
    let _this = this;
    $(this.getObject()).find("a").click(function() {
      $(_this.getObject()).find("li").removeClass("active");
  ...
  _eachNode(node) {
    ...
    if (this._firstUl) {
      ...
      ulCode = `<ul class = "JSTree ${this.objectClass}" id = "${this.getId()}">`;
      this._firstUl = false;
    } else {
      ulCode = "<ul>";
    }
    ...
    for (let i = 0; i < node.length; i++) {
      ...
      if (undefined != obj.link) {
        // 子菜单
        ulCode += `<li><a href = "${obj.link}" target = "${this.objectLinkTarget}">${iCode}${obj.text}</a></li>`;
      } else {
        // 父菜单
