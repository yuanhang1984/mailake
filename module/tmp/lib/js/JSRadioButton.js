...
class JSRadioButton {
  ...
  generateCode() {
    ...
    this.objectCode = `
      <input ${checkedCode} ${enabledCode} type = "radio" class = "JSRadioButton ${this.objectClass}" id = "${this.getId()}" name = "${this.objectName}" value = "${this.objectValue}" /><label class = "JSRadioButton ${this.objectClass}" for = "${this.getId()}">${this.objectLabel}</label>
    `;
  }
}
